...
describe("trigger/chat.ts is cut over to adviser-v3 (the shipped system prompt)", () => {
  it("wires ADVISER_V3, not a frozen older version, as the agent's system prompt", () => {
    const src = readFileSync(resolve(process.cwd(), "trigger/chat.ts"), "utf8");
    expect(src).toMatch(/system:\s*ADVISER_V3\b/);
    expect(src).not.toMatch(/system:\s*ADVISER_V[12]\b/);
  });
});
